...
export async function routingWorkflow(query: string) {
  ...
  const { object: classification } = await generateObject({
    model: azure.responses('grok-4-fast-non-reasoning'),
    schema: z.object({
      reasoning: z.string(),
  ...
  const { text: response } = await generateText({
    model:
      classification.complexity === 'simple'
        ? azure.responses('grok-4-fast-non-reasoning')
        : azure.responses('grok-4-fast-reasoning'),
    system: {
      general:
